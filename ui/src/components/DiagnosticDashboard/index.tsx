...
const PROBLEM_TYPES = {
  semantic_incomplete: {
    ...
    description:
      "Chunk contains partial idea (dangling connectives, incomplete sentences)",
    color: "text-red-400",
  },
...
export function DiagnosticDashboard({ document }: DiagnosticDashboardProps) {
  ...
  const [statistics, setStatistics] = useState<DiagnosticStatistics | null>(
    null,
  );
  ...
  const previewFixes = async () => {
    ...
    try {
      ...
      setFixError(
        error instanceof Error ? error.message : "Failed to preview fixes",
      );
    } finally {
  ...
  const applyFixes = async () => {
    ...
    try {
      ...
      setFixError(
        error instanceof Error ? error.message : "Failed to apply fixes",
      );
    } finally {
    ...
    <div className="h-full flex flex-col bg-kiln-900">
      ...
      <div className="border-b-4 border-black p-4 bg-kiln-800">
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-pixel text-2xl text-kiln-300">
        ...
        {fixSuccess && (
          <div className="mt-4 bg-green-900/30 border-2 border-green-400 p-3">
            <h3 className="text-pixel text-green-400 mb-2">
          ...
          <div className="bg-kiln-900 border-2 border-black p-3">
            <div className="flex items-center gap-2">
              ...
              <div>
                ...
                <div className="text-pixel text-xl text-kiln-300">
                  {healthyChunks}
                </div>
              </div>
            </div>
          ...
          <div className="bg-kiln-900 border-2 border-black p-3">
            <div className="flex items-center gap-2">
              ...
              <div>
                ...
                <div className="text-pixel text-xl text-red-400">
                  {highSeverity}
                </div>
              </div>
            </div>
          ...
          <div className="bg-kiln-900 border-2 border-black p-3">
            <div className="flex items-center gap-2">
              ...
              <div>
                ...
                <div className="text-pixel text-xl text-orange-400">
                  {mediumSeverity}
                </div>
              </div>
            </div>
          ...
          <div className="bg-kiln-900 border-2 border-black p-3">
            <div className="flex items-center gap-2">
              ...
              <div>
                ...
                <div className="text-pixel text-xl text-yellow-400">
                  {lowSeverity}
                </div>
              </div>
            </div>
        ...
        <div className="w-1/3 border-r-4 border-black bg-kiln-800 overflow-y-auto flex flex-col">
          ...
          {problems.length > 0 ? (
            <div className="flex-1 overflow-y-auto p-4">
              <h2 className="text-pixel text-lg text-kiln-300 mb-4">
                DETECTED PROBLEMS
              </h2>
              <div className="space-y-2">
                {problems.map((problem, idx) => {
                  const typeInfo = PROBLEM_TYPES[
                    problem.problem_type as keyof typeof PROBLEM_TYPES
                  ] || {
                    label: problem.problem_type,
                    color: "text-kiln-500",
                  };
                  const chunk = document.chunks.find(
                    (c) => c.id === problem.chunk_id,
                  );

                  return (
                    <div
                      key={`${problem.id ?? `${problem.chunk_id}-${problem.problem_type}`}-${idx}`}
                    ...
                    >
                      ...
                      <p className="text-xs text-kiln-500 mb-2">
                        {problem.description}
                      </p>
                      {chunk && (
              ...
              {!statistics ? (
                ...
                <div className="text-center py-8">
                  ...
                  <p className="text-xs text-kiln-500 mb-4">
                    Click "RUN DIAGNOSTICS" above to check for problems
                  </p>
                ...
                <div className="text-center py-8">
                  ...
                  <div className="bg-green-900/20 border border-green-400 p-4 text-left max-w-md mx-auto">
                    ...
                    <p className="text-xs text-kiln-500 mt-3">
                      Your document was likely well-structured to begin with.
                      You can still test retrieval quality with custom queries
                      in the Test panel.
                    </p>
                  </div>
                </div>
              ...
              <div>
                <div className="flex items-center justify-between mb-4">
                  <div className="flex items-center gap-3">
                    ...
                    <h2 className="text-pixel text-lg text-kiln-300">
                      CHUNK DETAILS
                    </h2>
                  </div>
                  <button
                ...
                <div className="bg-kiln-800 border-2 border-black p-4 mb-4">
                  <div className="grid grid-cols-3 gap-4 mb-4">
                    ...
                    <div>
                      ...
                      <div className="text-xs text-kiln-300 truncate">
                ...
                {isAnnotating && (
                  <div className="bg-error/10 border-2 border-error p-4">
                    <h3 className="text-pixel text-error mb-4">
                      ANNOTATE PROBLEM
                    </h3>

                    <div className="space-y-4">
                      <div>
                        <label className="text-xs text-kiln-500 block mb-2">
                          PROBLEM TYPE
                        </label>
                ...
                <div className="mt-4">
                  <h3 className="text-pixel text-sm text-kiln-300 mb-2">
                    PROBLEMS FOR THIS CHUNK
                  </h3>
                  {problems.filter((p) => p.chunk_id === selectedChunk.id)
                    .length === 0 ? (
                    <p className="text-xs text-kiln-500">
                    ...
                    <div className="space-y-2">
                      {problems
                        .filter((p) => p.chunk_id === selectedChunk.id)
                        .map((problem, idx) => {
                          const typeInfo = PROBLEM_TYPES[
                            problem.problem_type as keyof typeof PROBLEM_TYPES
                          ] || {
                          ...
                          return (
                            <div
                              key={idx}
                              className="bg-kiln-800 border border-black p-3"
                            >
                              <div
                                className={`text-xs font-bold ${typeInfo.color} mb-1`}
                              >
