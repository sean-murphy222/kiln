              ...
              <ul className="space-y-1 ml-4">
                {best.hierarchy_preservation > 0.8 && (
                  <li>
                    ...
                    {(best.hierarchy_preservation * 100).toFixed(0)}% hierarchy)
                  </li>
                )}
                {best.avg_quality_score > 0.9 && (
                  <li>
                    • High quality chunks (score:{" "}
                    {best.avg_quality_score.toFixed(2)})
                  </li>
                )}
                {best.avg_tokens >= 300 && best.avg_tokens <= 600 && (
                  <li>
                    • Optimal token size ({best.avg_tokens.toFixed(0)} tokens
                    avg)
                  </li>
                )}
                {best.chunks_with_context / best.chunks_count > 0.9 && (
                  <li>
                    • Includes hierarchy context for {best.chunks_with_context}{" "}
                    chunks
                  </li>
                )}
              </ul>
