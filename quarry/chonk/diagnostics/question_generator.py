...
class QuestionGenerator:
    ...
    def __init__(self):
        ...
        self.backward_ref_patterns = [
            re.compile(p, re.IGNORECASE) for p in self.BACKWARD_REFERENCES
        ]
    ...
    def generate_boundary_questions(
        ...
        for connective in self.CONNECTIVES:
            if content.startswith(connective):
                ...
                questions.append(
                    GeneratedQuestion(
                        question=(
                            f"What is the complete context for the statement "
                            f"that {connective}...?"
                        ),
        ...
        if chunk.content.strip() and chunk.content.strip()[-1] not in ".!?":
            if next_chunk:
        ...
        for pattern in self.forward_ref_patterns:
            if pattern.search(chunk.content):
                questions.append(
                    GeneratedQuestion(
                        question=(
                            f"What information follows regarding "
                            f"{self._extract_subject(chunk)}?"
                        ),
                        ...
                        metadata={
                            ...
                            "note": (
                                ...
                                "may be orphaned if content is in next chunk"
                            ),
                        },
                    )
                )
        ...
        for pattern in self.backward_ref_patterns:
            if pattern.search(chunk.content):
                questions.append(
                    GeneratedQuestion(
                        question=(
                            f"What is the context and background for "
                            f"{self._extract_subject(chunk)}?"
                        ),
    ...
    def generate_structural_questions(
        ...
        if matches:
            ...
            if min_num > 1:
                ...
                questions.append(
                    GeneratedQuestion(
                        question=(
                            "What are all the items in the numbered list " "(starting from item 1)?"
                        ),
                        expected_chunk_ids=expected_chunks,
        ...
        for marker_pattern in procedural_markers:
            if re.search(marker_pattern, chunk.content, re.IGNORECASE):
                questions.append(
                    GeneratedQuestion(
                        question="What is the complete procedure or sequence of steps?",
    ...
    def generate_coherence_questions(
        ...
        if chunk.token_count > 500:
            ...
            if len(topics) > 10:  # Many topics = likely contaminated
                questions.append(
                    GeneratedQuestion(
                        question="What specific information is provided about [topic]?",
                        expected_chunk_ids=[chunk.id],
                        ...
                        metadata={
                            ...
                            "note": (
                                "Very large chunk with many topics - "
        ...
        if chunk.token_count < 20:
            questions.append(
                GeneratedQuestion(
                    question=(
                        f"What is the complete information about "
                        f"'{chunk.content.strip()[:50]}'?"
                    ),
                    expected_chunk_ids=[chunk.id],
    ...
    def _extract_subject(self, chunk: Chunk) -> str:
        ...
        for word in words:
            if (
                word
                and word[0].isupper()
                and word.lower() not in ["the", "a", "an", "in", "on", "at"]
            ):
